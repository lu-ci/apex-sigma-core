...
`>>combinechains` `>>combine` `>>cmbch` `>>mix` | Like the impersonate command. This one however targets all mentioned users and uses their combined chains to generate a response. You can specify either a character limit, or a starting state of one to two words (not both). At least one item in the combined chain must begin with the specified starting state for this to work. One word starting states are more likely to work than two word starting states. | `>>combinechains @person_one @person_two`
