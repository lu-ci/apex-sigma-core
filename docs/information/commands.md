...
`>>autopunish` `>>automod` | Toggles the auto punishment feature on or off. This command requires the Manage Server permission. | `>>autopunish @person`
`>>autopunishlevels` `>>automodlevels` | Sets the desired punishment for a certain number of warnings. Provide no arguments to display the current levels. This command requires the Manage Server permission. | `>>autopunishlevels @person`
...
`>>mutedusers` `>>listmuted` `>>mustedlist` | Displays a list of currently text-muted and hard-muted users, ass well as when those mutes expire, if timed. | `>>mutedusers`
