...
`>>bash` | If you are old enough to know what IRC is or remember what it looked like, then you will appreciate the quotes that the bash command produces. Personal favorite quote command. | `>>bash`
...
`>>raceoverride` `>>raceover` `>>killrace` | Overrides the race in the current channel in case a bug occurs. | `>>raceoverride`
...
`>>roulette` `>>roul` | Spin the roulette wheel and maybe you'll win. The default bet is 10 Kud but you can specify any amount you want. The syntax of playing is the bet amount first, it can be ommitted however, and then the roulette spot choice in the form of [selector]:[value]. Accepted specifiers are color:b or number:15, column:2, dozen:1, half:1, and type:odd. Good luck. | `>>roulette 150 color:red`
...
`>>ban` | Bans a user from the server. This will also remove all messages from that user in the last 24h. The user can only be mentioned by a mention tag. This is to preserve compatibility with logging and audit logs. This command requires the Ban permission. | `>>ban @person Way, WAY too spicy for us...`
`>>hardmute` `>>hmute` | Hard-mutes the mentioned user. Users who are hard-muted are disallowed from typing to any channel. There is no message deletion, this is a permission based mute. Due to adding a user override to every editable channel, this can spam audit logs. This command requires the Manage Channels permission. | `>>hardmute @person For talking about the fight club.`
...
`>>giverole` `>>giverank` `>>grole` `>>grank` | Gives the specified role to the mentioned user. The role must be below Sigma's highest role. This command requires the Manage Server permission. | `>>giverole @person Grandma`
...
`>>pruneroles` `>>proles` | Removes all empty roles from the server that are below Sigma. This command requires the Manage Server permission. | `>>pruneroles`
`>>removerole` `>>removerank` `>>rrole` `>>rrank` | Removes the specified role from the mentioned user. The role must be below Sigma's highest role. This command requires the Manage Server permission. | `>>removerole @person Wangly`
...
`>>togglerole` `>>togglerank` `>>rank` `>>trl` | Toggles a self assignable role. If you don't have the role, it will be given to you. If you do have the role, it will be removed from you. Multiple role can be specified by separating them with a semicolon and a space "; ". | `>>togglerole Overlord`
`>>toggleselfrole` `>>tsr` | The addselfrole and delselfrole in one. It toggles the self-assignability of a role. Multiple role can be specified by separating them with a semicolon and a space "; ". This command requires the Manage Roles permission. | `>>toggleselfrole Meat Lover`
...
`>>weather` `>>we` | Shows meteorological information about the specified location. You can add an unit argument at the end of the lookup: auto: automatically select units based on geographic location ca: same as si, except that windSpeed is in kilometers per hour uk2: same as si, except that nearestStormDistance and visibility are in miles and windSpeed is in miles per hour us: Imperial units (the default) si: SI units If no unit is selected it defaults to auto.  | `>>weather Belgrade unit:si`
...
`>>chatterbot` | Toggles if the Chatterbot functions should be active. If active, when a message starts with a mention of Sigma, she will respond. This setting is active by default. This command requires the Manage Server permission. | `>>chatterbot`
...
`>>decay` | Marks a channel as decaying with a given timer. Messages sent to decaying channels decay. This means that once they are sent, a timer starts, after the specified amount of time passes, the message is deleted automagically. This command requires the Manage Server permission. | `>>decay 1:30:59 #hotness`
...
`>>disabledecay` `>>undecay` | Disables a channel's decay. This command requires the Manage Server permission. | `>>disabledecay #hotnesslicious`
...
`>>greetmessage` `>>greetmsg` | This sets the message shown on the server when a member leaves. Greet messages can have special dynamic arguments in them. {user_name}     - Leaving user's name. {user_disc}     - "#xxxx" in the user's name. {user_nick}     - Message author's nickname. {user_mention}  - Tag the leaving user. {user_id}       - Leaving user's ID. {server_name}   - Server name. {server_id}     - Server ID. {owner_name}    - Server owner's name. {owner_disc}    - "#xxxx" in the server owner's name. {owner_nick}    - Server owner's nickname. {owner_mention} - Tag the server owner. {owner_id}      - Server owner's ID.  | `>>greetmessage Hello {user_mention}, welcome to {server_name}!`
`>>hardblockedwords` | Lists all hard-blocked words on the server. | `>>hardblockedwords`
`>>hardblockwords` `>>hardblockword` | Works like "blockwords" but very intolerable. For example if you hardblock the word "ass" it will delete stuff like "assassin". It looks for any instance of the contents in the message, not indivitial segments. If any of the words in the filter are exist within a message, whether as a standalone word or within another word, the message will be deleted and the author will be notified. Extensions should not be separated by a delimiter. Those with the Administrator permission are not affected. | `>>hardblockwords crap ass tits`
...
`>>removecommand` `>>deletecommand` `>>remcmd` `>>delcmd` | Removes a custom command trigger from the server. This requires the Manage Server permission. | `>>removecommand hi`
`>>removereactor` `>>deletereactor` `>>remreac` `>>delreac` | Removes an automatic reaction trigger. | `>>removereactor hi`
`>>removeresponder` `>>deleteresponder` `>>remres` `>>delres` | Removes a custom responder trigger from the server. This requires the Manage Server permission. | `>>removeresponder hi`
...
`>>emote` `>>emoji` `>>em` | Searches for an emoji by name. To make the search more precise you can add the ID of the emote or server ID that the emote is from, like in this example "monika:375824498882117635". | `>>emote Monika`
