...
`>>oserverinformation` `>>oserverinfo` `>>osinfo` | Shows information and data about the target server. The server lookup is by either name or guild ID. (Bot Owner Only) | `>>oserverinformation Lucia's Cipher`
...
`>>trivia` `>>triv` `>>t` | A trivia minigame. You are given a question and have to input the number of your answer. Guess correctly and you win 10-50 Kud (base), depending on the difficulty. You have 20 seconds to answer the question. Consecutive correct answers give you a streak and the streak increases the Kud gained from correct answers. Timing out or giving an incorrect answer resets the streak. You can choose the category and difficulty of the question like in the usage example, but choosing either will disable the streak. | `>>trivia easy games`
...
`>>echo` `>>say` | Makes the bot repeat what you entered, simple enough. | `>>echo Beep, bop-boop!`
...
`>>xbooru` `>>xb` | Searches Xbooru for the given tag. If no tag is given, the keyword "nude" will be used. As on all image galleries, tags are bound with a "+". If your search includes spaces, replace them with underscores "_". | `>>xbooru ovum`
...
`>>permittedchannels` `>>permedc` | Lists all channel overrides for the given command or module. Use the syntax {c|m}:{name} to specify if it's a command or a module, and its name. The list is paginated, each page has up to 50 channels. | `>>permittedchannels m:fun`
...
`>>antispam` | Toggles the antispam limiter. This basically enforces a custom rate limited upon users. The default is 5 messages per 5 seconds. | `>>antispam`
...
`>>capslimit` | Sets the minimum number of capital letters in a message to check for before a message is checked for what percentage of the message is in caps. | `>>capslimit 9`
`>>capspercentage` `>>capspercent` | Sets the minimum percentage of the message's capital letters. If this percentage is crossed, the message is deleted. Basically if 80% of the message is caps (as set in the usage example), it'll get nuked. | `>>capspercentage 80`
...
`>>ratelimit` | Sets the message amount and timespan for the rate limit. Separated by a slash, the amount goes first and timespan second. The usage example translates to a limit of 6 messages per 10 seconds. Any messages that cross it get nuked. | `>>ratelimit 6/10`
