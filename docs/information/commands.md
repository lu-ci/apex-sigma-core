...
`>>oserverinformation` `>>oserverinfo` `>>osinfo` | Shows information and data about the target server. The server lookup is by either name or ID. (Bot Owner Only) | `>>oserverinformation Lucia's Cipher`
