...
`>>quote` | Quotes a message from its given ID. The message must have text content or an attachment. Embeds cannot be quoted. | `>>quote 381449702589202432`
...
`>>starboard` `>>sb` | Toggles the starboard on and off. When a message reaches enough ractions with the specified emote, it will be posted to the starboard channel. You can set the channel, emote, and minimum number of reactions before a message is posted. | `>>starboard`
...
`>>starboardlimit` `>>sblimit` `>>sbl` | Sets the minimum number of reactons for a starboard to be posted. The lowest number you can set is 5. | `>>starboardlimit 25`
