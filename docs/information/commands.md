...
`>>bazaarstatistics` `>>bazaarstats` | Shows various statistics about your bazaar usage. Total currency spent, markup paid, average markup and multiplier, etc. If no user is targeted, the command will show statistics for the command author. | `>>bazaarstatistics @target`
