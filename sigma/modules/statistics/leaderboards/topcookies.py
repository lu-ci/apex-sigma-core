...
async def topcookies(cmd: SigmaCommand, message: discord.Message, args: list):
    ...
    if args:
        if args[0].lower() == 'total':
            sort_key = 'Total'
            lb_category = 'Total'
    all_docs = await cmd.db[cmd.db.db_cfg.database].Cookies.find({}).sort(sort_key, -1).limit(99).to_list(None)
    ...
    for data_doc in all_docs:
        ...
        if user_object:
            ...
            if len(leader_docs) >= 20:
                break
    table_data = [[doc[0].name[:12], str(doc[1])] for doc in leader_docs]
