...
async def drink(cmd: SigmaCommand, pld: CommandPayload):
    interaction = await grab_interaction(cmd.db, 'drink')
    target, auth = get_target(pld.msg.guild.me, pld.msg), get_author(pld.msg.guild.me, pld.msg)
    ender = 'drinks' if target.id == pld.msg.author.id else f'drinks with {target.display_name}'
    response = discord.Embed(color=0xffbc3f, title=f'🍻 {auth.display_name} {ender}.')
