...
async def sleep(cmd: SigmaCommand, pld: CommandPayload):
    interaction = await grab_interaction(cmd.db, 'sleep')
    target = get_target(pld.msg)
    auth = pld.msg.author
    if not target or target.id == pld.msg.author.id:
        response = discord.Embed(color=0xffcc4d, title=f'😴 {auth.display_name} sleeps by themself.')
    else:
        response = discord.Embed(color=0xffcc4d, title=f'😴 {auth.display_name} sleeps next to {target.display_name}.')
