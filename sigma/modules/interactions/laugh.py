...
async def laugh(cmd: SigmaCommand, pld: CommandPayload):
    ...
    if not target or target.id == pld.msg.author.id:
        response = discord.Embed(color=0xffcc4d, title=f'😆 {auth.display_name} laughs at themself.')
    else:
        response = discord.Embed(color=0xffcc4d, title=f'😆 {auth.display_name} laughs at {target.display_name}.')
