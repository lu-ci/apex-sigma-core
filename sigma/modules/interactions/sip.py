...
async def sip(cmd, pld):
    ...
    ender = 'sips their beverage' if target.id == pld.msg.author.id else f'takes a sip with {target.display_name}'
    response = discord.Embed(color=0xa6d388, title=f'🍵 {auth.display_name} {ender}.')
