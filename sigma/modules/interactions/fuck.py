...
async def fuck(cmd: SigmaCommand, pld: CommandPayload):
    ...
    ender = 'fucks' if target.id == pld.msg.author.id else f'fucks {target.display_name}'
    response = discord.Embed(color=0x744EAA, title=f'🍆 {auth.display_name} {ender}.')
