...
async def pout(cmd: SigmaCommand, pld: CommandPayload):
    interaction = await grab_interaction(cmd.db, 'pout')
    target, auth = get_target(pld.msg.guild.me, pld.msg), get_author(pld.msg.guild.me, pld.msg)
    ender = 'pouts' if target.id == pld.msg.author.id else f'pouts at {target.display_name}'
