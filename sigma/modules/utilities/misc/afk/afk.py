...
async def afk(cmd, pld):
    ...
    if not afk_data:
        afk_data = await cmd.db.col.AwayUsers.find_one({'user_id': pld.msg.author.id})
    if pld.args:
    ...
    await cmd.db.col.AwayUsers.update_one({'user_id': pld.msg.author.id}, {'$set': update_data}, upsert=True)
