...
async def wffissurechannel(cmd, pld):
    """
    ...
    if pld.msg.channel.permissions_for(pld.msg.author).manage_channels:
        if pld.msg.channel_mentions:
            ...
            if pld.args:
                if pld.args[0].lower() == 'disable':
                    await cmd.db.set_guild_settings(pld.msg.guild.id, 'warframe_fissure_channel', None)
                    response = GenericResponse('Warframe Void Fissure Channel disabled.').ok()
        ...
        response = GenericResponse(f'Warframe Void Fissure Channel set to #{target_channel.name}').ok()
    else:
