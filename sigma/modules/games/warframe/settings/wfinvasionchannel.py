...
async def wfinvasionchannel(cmd, pld):
    """
    ...
    if pld.msg.channel.permissions_for(pld.msg.author).manage_channels:
        if pld.msg.channel_mentions:
            ...
            if pld.args:
                if pld.args[0].lower() == 'disable':
                    ...
                    response = GenericResponse('Warframe Invasion Channel disabled.').ok()
        ...
        response = GenericResponse(f'Warframe Invasion Channel set to #{target_channel.name}').ok()
    else:
