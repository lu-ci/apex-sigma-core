...
async def get_ship(db, lookup):
    ...
    :rtype: dict
    """
    ship = await db.col.AzurLaneShips.find_one({'id': lookup})
    if ship is None:
        ship = await db.col.AzurLaneShips.find_one({'name': lookup.title()})
        if ship is None:
            all_ships = await db.col.AzurLaneShips.find().to_list(None)
