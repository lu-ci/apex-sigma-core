...
async def fill_body_armor_cache():
    if not body_armor_list_cache:
        active_sg_url = 'https://pathofexile.gamepedia.com/Body_Armour'
...
async def poebody(cmd: SigmaCommand, message: discord.Message, args: list):
    if args:
        ...
        if armor_entry:
            ...
            if armor_entry:
                ...
                response = discord.Embed(color=0xBE1931, title='❗ Invalid armor data received.')
        else:
            response = discord.Embed(color=0xBE1931, title='❗ Armor not found.')
    else:
