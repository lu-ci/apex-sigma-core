...
glove_list_cache = {}
glove_data_cache = {}
glove_icon = 'https://i.imgur.com/6fDDMqO.png'
item_urls = ['https://pathofexile.gamepedia.com/Gloves', 'https://pathofexile.gamepedia.com/List_of_unique_gloves']
        ...
        for active_sg_url in item_urls:
            ...
            for armor_item in armor_items:
                ...
                if armor_name:
                    ...
                    glove_list_cache.update({armor_dkey: {'name': armor_name, 'url': armor_link}})


async def get_armor_data(armor_name: str, armor_url: str):
    armor_key = armor_name.replace(' ', '_').lower()
    armor_data = glove_data_cache.get(armor_key)
    if not armor_data:
        async with aiohttp.ClientSession() as session:
        ...
        glove_data_cache.update({armor_key: armor_data})
    return armor_data


def find_broad(lookup: str):
    out = None
    for key in glove_list_cache:
        if lookup in key:
            out = glove_list_cache.get(key)
            break
    return out
...
async def poegloves(_cmd: SigmaCommand, pld: CommandPayload):
    if pld.args:
        lookup_key = '_'.join(pld.args).lower()
        await fill_gloves_cache()
        armor_entry = glove_list_cache.get(lookup_key) or find_broad(lookup_key)
        if armor_entry:
            ...
            if armor_entry:
                ...
                response = error('Invalid glove data received.')
        else:
            response = not_found('Gloves not found.')
    else:
