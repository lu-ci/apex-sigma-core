...
helmet_list_cache = {}
helmet_data_cache = {}
helmet_icon = 'https://i.imgur.com/AFQXzZu.png'
item_urls = ['https://pathofexile.gamepedia.com/Helmets', 'https://pathofexile.gamepedia.com/List_of_unique_helmets']


async def fill_helmets_cache():
    """

    """
    if not helmet_list_cache:
        for active_sg_url in item_urls:
            ...
            for armor_item in armor_items:
                ...
                if armor_name:
                    ...
                    helmet_list_cache.update({armor_dkey: {'name': armor_name, 'url': armor_link}})
...
async def get_armor_data(armor_name: str, armor_url: str):
    ...
    armor_data = helmet_data_cache.get(armor_key)
    if not armor_data:
        async with aiohttp.ClientSession() as session:
        ...
        helmet_data_cache.update({armor_key: armor_data})
    return armor_data
...
def find_broad(lookup: str):
    ...
    """
    out = None
    for key in helmet_list_cache:
        if lookup in key:
            out = helmet_list_cache.get(key)
            break
    return out
    ...
    if pld.args:
        ...
        if armor_entry:
            ...
            if armor_entry:
                ...
                response = error('Invalid helmet data received.')
        else:
            response = not_found('Helmet not found.')
    else:
