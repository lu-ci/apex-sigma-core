...
async def greetchannel(cmd: SigmaCommand, message: discord.Message, _args: list):
    if message.author.permissions_in(message.channel).manage_guild:
        if message.channel_mentions:
            target_channel = message.channel_mentions[0]
            if message.guild.me.permissions_in(target_channel).send_messages:
                await cmd.db.set_guild_settings(message.guild.id, 'greet_channel', target_channel.id)
