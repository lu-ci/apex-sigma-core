...
async def greetembed(cmd: SigmaCommand, message: discord.Message, args: list):
        ...
        if args:
            ...
            for qry in qrys:
                if qry[1]:
                    ...
                    return
            await cmd.db.set_guild_settings(message.guild.id, 'greet_embed', embed_data)
            response = discord.Embed(color=0x77B255, title=f'✅ Greeting Embed updated.')
            ...
            await cmd.db.set_guild_settings(message.guild.id, 'greet_embed', embed_data)
            response = discord.Embed(color=0x77B255, title=f'✅ Greeting Embed {ender}.')
    else:
