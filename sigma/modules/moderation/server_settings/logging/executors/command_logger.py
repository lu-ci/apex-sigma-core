...
async def command_logger(ev, cmd, message, args):
    if message.guild:
        ...
        log_channel_id = await ev.db.get_guild_settings(message.guild.id, 'LoggingChannel')
        logged_modules = await ev.db.get_guild_settings(message.guild.id, 'LoggedModules') or []
