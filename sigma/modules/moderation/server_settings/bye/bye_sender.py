...
async def bye_sender(ev: SigmaEvent, pld: MemberPayload):
    bye_active = await ev.db.get_guild_settings(pld.member.guild.id, 'bye')
    bye_active = True if bye_active is None else bye_active
    if bye_active:
        bye_channel_id = await ev.db.get_guild_settings(pld.member.guild.id, 'bye_channel')
        if bye_channel_id:
            target = pld.member.guild.get_channel(bye_channel_id)
        else:
            target = None
        if target:
            current_goodbye = await ev.db.get_guild_settings(pld.member.guild.id, 'bye_message')
            ...
            bye_embed = await ev.db.get_guild_settings(pld.member.guild.id, 'bye_embed') or {}
