...
async def check_emotes(db, mid, uid, sbl):
    ...
    :rtype: bool
    """
    cache = await db.col.StarboardCache.find_one({'message_id': mid})
    if not cache:
        cache = {
            'message_id': mid,
    ...
    if not cache.get('executed'):
        ...
        await db.col.StarboardCache.update_one({'message_id': mid}, {'$set': cache}, upsert=True)
    return trigger
...
async def starboard_watcher(ev, pld):
    ...
    try:
        ...
        if file or embed:
            ...
            await ev.db.col.StarboardCache.delete_many({'created': {'$lt': cutoff}})
