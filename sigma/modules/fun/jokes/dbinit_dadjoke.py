...
async def dbinit_dadjoke(ev, force=False):
    """
    :param ev: The event object referenced in the event.
    :type ev: sigma.core.mechanics.event.SigmaEvent
    :type force: bool
    """
    doc_count = await ev.db[ev.db.db_name].DadjokeData.count_documents({})
    if not doc_count or force:
        ...
        await ev.db[ev.db.db_name].DadjokeData.drop()
        async with aiohttp.ClientSession() as session:
            async with session.get(file_url) as data_response:
                data = await data_response.read()
                data = yaml.safe_load(data)
        await ev.db[ev.db.db_name].DadjokeData.insert_many(data.get('JOKES'))
