...
async def dbinit_fortune(ev: SigmaEvent, force=False):
    ...
    if not doc_count or force:
        file_url = 'https://gitlab.com/lu-ci/apex-sigma-res/raw/master/jokes/fortune_manifest.yml'
        ev.log.info('Updating fortune-mod files.')
        await ev.db[ev.db.db_nam].FortuneData.drop()
        documents = []
        ...
        await ev.db[ev.db.db_nam].FortuneData.insert_many(documents)
