...
async def givetovault(cmd: SigmaCommand, message: discord.Message, args: list):
    if args:
        ...
        if amount:
            ...
            current_kud = await cmd.db.get_currency(message.author, message.guild, True)
            ...
            if current_kud >= amount:
                ...
                if current_vault is None:
                    current_vault = 0
                await cmd.db.rmv_currency(message.author, amount, True)
                ...
                response = discord.Embed(color=0xa7d28b, title=f'💸 You don\'t have that much tradable {currency}.')
        else:
