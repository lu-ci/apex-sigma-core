...
async def soundslike(_cmd: SigmaCommand, message: discord.Message, args: list):
    if args:
        ...
        if data:
            ...
            response.set_author(name=f'Words that sound like {query.replace("+", " ")}', url=site_url, icon_url=icon)
            if len(data) > 10:
