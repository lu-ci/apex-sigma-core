...
async def rhymes(_cmd: SigmaCommand, message: discord.Message, args: list):
    if args:
        ...
        if data:
            ...
            response.set_author(name=f'Words that rhyme with {query.replace("+", " ")}', url=site_url, icon_url=icon)
            if len(data) > 10:
