...
async def invite_reporter_cycler(ev):
    ...
    while True:
        if ev.bot.is_ready():
            invite_docs = await ev.db.col.InviteQueue.find({'reported': False}).to_list(None)
            for invite_doc in invite_docs:
                ...
                if guild:
                    ...
                    await ev.db.col.InviteQueue.update_one(invite_doc, update_dict)
                    data = {
                        'reported': False,
                    ...
                    await ev.db.col.SystemMessages.insert_one(data)
